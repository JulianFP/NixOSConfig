...
class selectMenu:
    ...
    def __rawread(self, numberOfChars, fd, oldTerminalSettings, newTerminalSettings) -> str:
        try:
            ...
            print(
                "\x1b[{0}B".format(self.promptLines), end=""
            )  # make sure prompt gets not overwritten
    ...
    def wait(self):
        ...
        while key != "q":
            ...
            if key == "\x1b":
                key += self.__rawread(
                    2, self.fd, self.oldTerminalSettings, self.newTerminalSettings
                )
...
def addNebulaUnsafeRouteFor(routeName: str, selected: bool):
    ...
    try:
        subprocess.run(
            ["systemctl", "restart", "nebula-custom_serverNetwork.service"],
