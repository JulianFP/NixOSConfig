...
in 
{
  ...
  config = lib.mkIf (enabledContainers != {}) {
    ...
    services.nebula.networks = (builtins.mapAttrs (n: v:  {
    ...
    systemd = lib.mkMerge (lib.mapAttrsToList (n: v: let
    ...
    in {
      services = {
        ...
        "veth@${n}" = {
          ...
          serviceConfig = {
            ...
            ExecStart =
              let
                start = with pkgs; writeShellScript "veth-${n}-up" ''
                  set -e
        ...
        in {
          ...
          serviceConfig = with pkgs; lib.mkMerge [ nebulaSystemdService.serviceConfig {
            ExecStart = lib.mkForce "${iproute2}/bin/ip netns exec ${shortenedN} ${nebulaSystemdService.serviceConfig.ExecStart}";
            CapabilityBoundingSet = lib.mkForce [ "CAP_SYS_ADMIN" "CAP_NET_ADMIN" ];
            AmbientCapabilities = lib.mkForce [ "CAP_SYS_ADMIN" "CAP_NET_ADMIN" ];
        ...
        "moveNebNS@${n}" = {
          ...
          in {
            ...
            RemainAfterExit = true;
            ExecStart = let
                start = with pkgs; writeShellScript "moveNebNS-${n}" ''
                  set -e
      ...
      tmpfiles.settings."10-containerMountDirs" = lib.optionalAttrs v.enableSops {
        "/persist/sops-nix/${n}"."d" = {
          user = "root";
          group = "root";
          mode = "0755";
        };
      } // lib.mkMerge (lib.mapAttrsToList (n: v: {
        "${v.hostPath}"."d" = {
          user = "root";
          group = "root";
          mode = "0755";
    ...
    containers = builtins.mapAttrs (n: v: let
    ...
    in {
      ...
      networkNamespace = if v.nebulaOnly then "/run/netns/neb-${shortenedN}" else "/run/netns/${shortenedN}";
      bindMounts = lib.optionalAttrs v.enableSops {
        "/persist/sops-nix" = {
