...
let
  ...
  genConfig = name: value@{stable ? true, ...}: let
    nixpkgs = if stable then inputs.nixpkgs-stable else inputs.nixpkgs;
  in (mkConfig ({ inherit inputs nixpkgs stable; hostName = name; } // (lib.filterAttrs (n: v: n != "format") value))) // {format = value.format;};
...
in toGenSystems (builtins.groupBy (builtins.getAttr "system") (lib.attrsets.mapAttrsToList genConfig systems))
